...
// 잔상 털기. 부분 갱신을 몇 번 하면 배경부터 통째로 다시 그릴지.
//
// 제조사는 "부분 갱신 5~6번마다 한 번" 이라고 한다. 그런데 그건 화면 전체
// 내용이 바뀌는 경우(전자책 페이지 넘기기 같은)를 두고 한 말이다.
// 우리는 시계 칸이 화면의 3%, 항적 띠가 18% 다. 나머지는 아예 안 움직인다.
// 그러니 그 숫자를 그대로 가져다 쓸 이유가 없다.
//
// 얼마가 맞는지는 재봐야 안다. 지금은 꺼두고 관찰 중이다.
// 시리얼로 G<숫자> 를 보내면 바꿀 수 있다. G300 이면 300번마다.
static int  ghostEvery = 999999;      // 사실상 끔
