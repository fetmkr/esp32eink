...
class EpdMixed : public GxEPD2_426_GDEQ0426T82
{
  public:
    ...
    // 화면 메모리 두 장을 같은 값으로 채운다.
    //
    // 왜 필요한가 (실측으로 확인):
    //   흑백 부분 갱신은 창을 정해도 화면 전체를 훑는다. 창은 데이터를 쓰는
    //   자리일 뿐이다. 실제로 움직이는 픽셀은 두 메모리 값이 다른 곳뿐이다.
    //   (창을 480줄에서 60줄로 줄여도 갱신 시간이 407ms 로 같았다)
    //
    //   회색으로 그린 뒤에는 메모리에 회색용 값이 들어 있다. 그 값을 흑백
    //   기준으로 읽으면 두 장이 서로 다른 곳이 많아서, 시계 칸만 밀려고 해도
    //   화면 전체가 다시 칠해진다.
    //
    //   그래서 두 장을 같은 값으로 채워둔다. 그러면 "바뀐 곳 없음" 이 되어
    //   아무 데도 안 움직인다. 눈에 보이는 회색 그림은 그대로 남는다.
